...
function [Z,output]=GraphEncoderOri(G,Y,U,opts)
warning ('off','all');
if nargin<2
    Y={2};
end
...
for j=1:numY
    tmpY=Y{j};
    [tmpY,indT{j},K,n,nk{j},indK]=ProcessLabel(tmpY,n); % process label
    ratio=sum(indT{j})/n;
    for i=1:numG
        tmpG=G{i};tmpnk=nk{j};
        ZY=GraphEncoderEmbed(tmpG,tmpY,n,tmpnk,opts); % graph encoder embed
%         ZY=EncoderClassifier(ZY,tmpnk,indK);
        tmpS=0;
        ZStd{i,j}=std(ZY);
...
%% process labels
function [Y,indTrn,K,n,nk,indK]=ProcessLabel(Y,n)
[numN,numY]=size(Y);
if numN==1 && numY==1
    if Y<2 || floor(Y)~=Y
