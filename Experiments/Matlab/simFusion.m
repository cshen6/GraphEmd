...
if choice==7
    load('Letter.mat')
     spec=0;
    opts = struct('Adjacency',1,'DiagAugment',1,'Laplacian',0,'Spectral',spec,'LDA',0,'GNN',0,'knn',5,'dim',30);
    Acc1=zeros(rep,12);Acc2=zeros(rep,12);Time1=zeros(rep,12);Time2=zeros(rep,12);
    if spec>0
    ...
    for i=1:rep
        ...
        for j=1:3
            ...
            opts.indices=indices;
            lim=size(Label,1);
            ind=(Edge1(:,1)>lim) | (Edge1(:,2)>lim);Edge1=Edge1(~ind,:);
            ind=(Edge2(:,1)>lim) | (Edge2(:,2)>lim);Edge2=Edge2(~ind,:);
            ind=(Edge3(:,1)>lim) | (Edge3(:,2)>lim);Edge3=Edge3(~ind,:);
            tmp=GraphEncoderEvaluate(Edge1,Label); Acc1(i,1+(j-1)*4)=tmp{1,2};Acc2(i,1+(j-1)*4)=tmp{1,4};Time1(i,1+(j-1)*4)=tmp{4,2};Time2(i,1+(j-1)*4)=tmp{4,4};
            tmp=GraphEncoderEvaluate(Edge2,Label); Acc1(i,2+(j-1)*4)=tmp{1,2};Acc2(i,2+(j-1)*4)=tmp{1,4};Time1(i,2+(j-1)*4)=tmp{4,2};Time2(i,2+(j-1)*4)=tmp{4,4};
            tmp=GraphEncoderEvaluate(Edge3,Label); Acc1(i,3+(j-1)*4)=tmp{1,2};Acc2(i,3+(j-1)*4)=tmp{1,4};Time1(i,3+(j-1)*4)=tmp{4,2};Time2(i,3+(j-1)*4)=tmp{4,4};
            tmp=GraphEncoderEvaluate({Edge1,Edge2,Edge3},Label); Acc1(i,4+(j-1)*4)=tmp{1,2};Acc2(i,4+(j-1)*4)=tmp{1,4};Time1(i,4+(j-1)*4)=tmp{4,2};Time2(i,4+(j-1)*4)=tmp{4,4};
        end
    end
