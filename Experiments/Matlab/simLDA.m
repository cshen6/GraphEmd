...
%             Dist1='Euclidean';nn=100; %D=X*X' & false normalization can also work
        case 1
            Dist1='Euclidean';nn=50; type=4;d=100;%D=X*X' & false normalization can also work
        case 2
...
if choice==9
    tl = tiledlayout(3,2);
    opts = struct('Normalize',true);fs=32;Dist1='Euclidean';
    [X,Label]=simGenerateDis(4,1000,3,100);
    D=squareform(pdist(X, Dist1));D=max(max(D))-D;
    Z=GraphEncoder(D,Label,0,opts);Y=Label;
...
%     D=X*X';
    ...
    nexttile(tl)
    load('GEELDASim1Spec0.mat'); i=10;myColor2 = brewermap(11,'RdYlGn'); lw=3;
...
%             load('20NewsHome.mat');X=fea;Label=gnd; D=X*X';
        ...
        case 12
            load('Isolet.mat');X=fea;Label=gnd; Dist1='spearman';D = 1-squareform(pdist(X, Dist1));%?
        case 13
...
%             load('Wiki_Data.mat');D=GF; X=ASE(D,30);
        case 17
            load('smartphone.mat');D=edge2adj(Edge); X=ASE(D,30);
%         case 13
