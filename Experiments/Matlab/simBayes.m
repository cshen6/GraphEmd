function simBayes(choice,spec, rep)
...
% LDA Bayes, true bayes, LDA
% load('Wiki_Data.mat');
% K=max(Label);
% [Z,out]=GraphEncoder(1-TE,Label);
% nk=horzcat(out.nk);
% entro=horzcat(out.ClassMean);
% dimCh=horzcat(out.DimChoice);
% nk2=nk(dimCh);entro=entro(:,dimCh);
% classes=log(entro).*nk2*Z'+log(1-entro).*nk2*(1-Z)'+log(nk(1:K)');
% classes=softmax(classes);
% tt = vec2ind(classes)';
% mean(tt~=Label)
% mdl=fitcknn(Z,Label,'Distance','Euclidean','NumNeighbors',5);
% tt=predict(mdl,Z);
% mean(tt~=Label)
% 
% nk=horzcat(out.nk);
...
% std2=sum(std1.*(nk-1)/(n-K));
% classes=mean1.*std2.^-1*Z'-0.5*diag(mean1.*std2.^-1*mean1')+log(nk(1:K)');
% tt = vec2ind(classes)';
% mean(tt~=Label)
...
if choice==1 || choice==2
    ...
    for i=1:lim
        for r=1:rep
            ...
            Acc3(i,1)=Acc3(i,1)+G3{i,r}{1,1}/rep;Acc3(i,2)=Acc3(i,2)+G3{i,r}{1,2}/rep;Acc3(i,3)=Acc3(i,3)+G3{i,r}{1,3}/rep;
            Acc3(i,4)=Acc3(i,4)+G3{i,r}{4,1}/rep;Acc3(i,5)=Acc3(i,5)+G3{i,r}{4,2}/rep;Acc3(i,6)=Acc3(i,6)+G3{i,r}{4,3}/rep;
            Acc12(i,1)=Acc12(i,1)+G12{i,r}{1,1}/rep;Acc12(i,2)=Acc12(i,2)+G12{i,r}{1,2}/rep;Acc12(i,3)=Acc12(i,3)+G12{i,r}{1,3}/rep;
            Acc12(i,4)=Acc12(i,4)+G12{i,r}{4,1}/rep;Acc12(i,5)=Acc12(i,5)+G12{i,r}{4,2}/rep;Acc12(i,6)=Acc12(i,6)+G12{i,r}{4,3}/rep;
            Acc13(i,1)=Acc13(i,1)+G13{i,r}{1,1}/rep;Acc13(i,2)=Acc13(i,2)+G13{i,r}{1,2}/rep;Acc13(i,3)=Acc13(i,3)+G13{i,r}{1,3}/rep;
            Acc13(i,4)=Acc13(i,4)+G13{i,r}{4,1}/rep;Acc13(i,5)=Acc13(i,5)+G13{i,r}{4,2}/rep;Acc13(i,6)=Acc13(i,6)+G13{i,r}{4,3}/rep;
            Acc23(i,1)=Acc23(i,1)+G23{i,r}{1,1}/rep;Acc23(i,2)=Acc23(i,2)+G23{i,r}{1,2}/rep;Acc23(i,3)=Acc23(i,3)+G23{i,r}{1,3}/rep;
            Acc23(i,4)=Acc23(i,4)+G23{i,r}{4,1}/rep;Acc23(i,5)=Acc23(i,5)+G23{i,r}{4,2}/rep;Acc23(i,6)=Acc23(i,6)+G23{i,r}{4,3}/rep;
            Acc123(i,1)=Acc123(i,1)+G123{i,r}{1,1}/rep;Acc123(i,2)=Acc123(i,2)+G123{i,r}{1,2}/rep;Acc123(i,3)=Acc123(i,3)+G123{i,r}{1,3}/rep;
            Acc123(i,4)=Acc123(i,4)+G123{i,r}{4,1}/rep;Acc123(i,5)=Acc123(i,5)+G123{i,r}{4,2}/rep;Acc123(i,6)=Acc123(i,6)+G123{i,r}{4,3}/rep;
        end
    end
...
if choice==3
    ...
    for i=1:lim
        for r=1:rep
            ...
            Acc3(i,1)=Acc3(i,1)+G3{i,r}{1,1}/rep;Acc3(i,2)=Acc3(i,2)+G3{i,r}{1,2}/rep;Acc3(i,3)=Acc3(i,3)+G3{i,r}{1,3}/rep;
            Acc3(i,4)=Acc3(i,4)+G3{i,r}{4,1}/rep;Acc3(i,5)=Acc3(i,5)+G3{i,r}{4,2}/rep;Acc3(i,6)=Acc3(i,6)+G3{i,r}{4,3}/rep;
            Acc4(i,1)=Acc4(i,1)+G4{i,r}{1,1}/rep;Acc4(i,2)=Acc4(i,2)+G4{i,r}{1,2}/rep;Acc4(i,3)=Acc4(i,3)+G4{i,r}{1,3}/rep;
            Acc4(i,4)=Acc4(i,4)+G4{i,r}{4,1}/rep;Acc4(i,5)=Acc4(i,5)+G4{i,r}{4,2}/rep;Acc4(i,6)=Acc4(i,6)+G4{i,r}{4,3}/rep;
        end
    end
...
if choice==4 || choice==5 || choice==6 
    ...
    ind=1;ind3=3;ind2=2;%1: bayes, 2: NN, 3: LDA.
...
%         i
    ...
    save(strcat('GEEBayes',num2str(choice),'Spec',num2str(spec),'.mat'),'choice','Acc1','Acc2','Acc3','Time1','Time2','Time3');
...
%             lim=length(Label);
            ...
            indices = crossvalind('Kfold',Label,5);
            opts.indices=indices;
            tmp=GraphEncoderEvaluate(Edge1,Label,opts); Acc1(i,1+(j-1)*spc)=tmp{1,ind};Acc2(i,1+(j-1)*spc)=tmp{1,ind2};Time1(i,1+(j-1)*spc)=tmp{4,ind};Time2(i,1+(j-1)*spc)=tmp{4,ind2};
            tmp=GraphEncoderEvaluate(Edge2,Label,opts); Acc1(i,2+(j-1)*spc)=tmp{1,ind};Acc2(i,2+(j-1)*spc)=tmp{1,ind2};Time1(i,2+(j-1)*spc)=tmp{4,ind};Time2(i,2+(j-1)*spc)=tmp{4,ind2};
            tmp=GraphEncoderEvaluate(Edge3,Label,opts); Acc1(i,3+(j-1)*spc)=tmp{1,ind};Acc2(i,3+(j-1)*spc)=tmp{1,ind2};Time1(i,3+(j-1)*spc)=tmp{4,ind};Time2(i,3+(j-1)*spc)=tmp{4,ind2};
            tmp=GraphEncoderEvaluate({Edge1,Edge2,Edge3},Label,opts); Acc1(i,4+(j-1)*spc)=tmp{1,ind};Acc2(i,4+(j-1)*spc)=tmp{1,ind2};Time1(i,4+(j-1)*spc)=tmp{4,ind};Time2(i,4+(j-1)*spc)=tmp{4,ind2};
...
if choice==8
    load('Wiki_Data.mat');
    opts = struct('Adjacency',1,'DiagAugment',1,'Laplacian',0,'Spectral',spec,'LDA',1,'GNN',1,'knn',5,'dim',30);
    % opts2=opts;
    ...
    for i=1:rep
        ...
        indices = crossvalind('Kfold',Label,5);
        opts.indices=indices;
        tmp=GraphEncoderEvaluate(1-TE,Label,opts);Acc1(i,1)=tmp{1,ind};Acc2(i,1)=tmp{1,ind2};Time1(i,1)=tmp{4,2};Time2(i,1)=tmp{4,ind2};
        tmp=GraphEncoderEvaluate(1-TF,Label,opts);Acc1(i,2)=tmp{1,ind};Acc2(i,2)=tmp{1,ind2};Time1(i,2)=tmp{4,ind};Time2(i,2)=tmp{4,ind2};
        tmp=GraphEncoderEvaluate(GE,Label,opts);Acc1(i,3)=tmp{1,ind};Acc2(i,3)=tmp{1,ind2};Time1(i,3)=tmp{4,ind};Time2(i,3)=tmp{4,ind2};
        tmp=GraphEncoderEvaluate(GF,Label,opts);Acc1(i,4)=tmp{1,ind};Acc2(i,4)=tmp{1,ind2};Time1(i,4)=tmp{4,ind};Time2(i,4)=tmp{4,ind2};
        tmp=GraphEncoderEvaluate({1-TE,1-TF},Label,opts);Acc1(i,5)=tmp{1,ind};Acc2(i,5)=tmp{1,ind2};Time1(i,5)=tmp{4,ind};Time2(i,5)=tmp{4,ind2};
        tmp=GraphEncoderEvaluate({GE,GF},Label,opts);Acc1(i,6)=tmp{1,ind};Acc2(i,6)=tmp{1,ind2};Time1(i,6)=tmp{4,ind};Time2(i,6)=tmp{4,ind2};
        tmp=GraphEncoderEvaluate({GE,1-TE},Label,opts);Acc1(i,7)=tmp{1,ind};Acc2(i,7)=tmp{1,ind2};Time1(i,7)=tmp{4,ind};Time2(i,7)=tmp{4,ind2};
        tmp=GraphEncoderEvaluate({GF,1-TF},Label,opts);Acc1(i,8)=tmp{1,ind};Acc2(i,8)=tmp{1,ind2};Time1(i,8)=tmp{4,ind};Time2(i,8)=tmp{4,ind2};
        tmp=GraphEncoderEvaluate({1-TE,1-TF,GE},Label,opts);Acc1(i,9)=tmp{1,ind};Acc2(i,9)=tmp{1,ind2};Time1(i,9)=tmp{4,ind};Time2(i,9)=tmp{4,ind2};
        tmp=GraphEncoderEvaluate({1-TE,1-TF,GF},Label,opts);Acc1(i,10)=tmp{1,ind};Acc2(i,10)=tmp{1,ind2};Time1(i,10)=tmp{4,ind};Time2(i,10)=tmp{4,ind2};
        tmp=GraphEncoderEvaluate({GE,GF,1-TE},Label,opts);Acc1(i,11)=tmp{1,ind};Acc2(i,11)=tmp{1,ind2};Time1(i,11)=tmp{4,ind};Time2(i,11)=tmp{4,ind2};
        tmp=GraphEncoderEvaluate({GE,GF,1-TF},Label,opts);Acc1(i,12)=tmp{1,ind};Acc2(i,12)=tmp{1,ind2};Time1(i,12)=tmp{4,ind};Time2(i,12)=tmp{4,ind2};
        tmp=GraphEncoderEvaluate({1-TE,1-TF,GE,GF},Label,opts);Acc1(i,13)=tmp{1,ind};Acc2(i,13)=tmp{1,ind2};Time1(i,13)=tmp{4,ind};Time2(i,13)=tmp{4,ind2};
    end
...
if choice>=10 && choice<=25
    switch choice
        ...
        case 12
            load('protein.mat');
        case 13
            load('COIL-RAG.mat'); %not work
        case 14
            load('adjnoun.mat');Label=Y;Edge=Edge+1;
        case 15
            load('email.mat');Label=Y;
        case 16
            load('Gene.mat');Label=Y;
        case 17
            load('IIP.mat');Label=Y;
        case 18
            load('polblogs.mat');Label=Y;
        case 19
            load('PubMed.mat');
        case 20
            load('CElegans.mat');Edge=Ac;Label=vcols;
        case 21
            load('CElegans.mat');Edge=Ag;Label=vcols;
        case 22
            load('IMDB.mat');Edge=Edge1;Label=Label1;
        case 23
            load('IMDB.mat');Edge=Edge2;Label=Label2;
%         case 14
...
%             load('AIDS.mat');K2=30;Dist2='sqeuclidean';Dist1='euclidean';D = 1-squareform(pdist(X, Dist1));
    ...
    Acc1=zeros(rep,4);Acc2=zeros(rep,4);Acc3=zeros(rep,4);Time1=zeros(rep,4);Time2=zeros(rep,4);Time3=zeros(rep,4);
    if spec>0
        Edge=edge2adj(Edge);
    end
    ind=1;ind3=3;ind2=2;%1: bayes, 2: NN, 3: LDA.
...
%        tmp=GraphEncoderEvaluate(D,Label,opts); Acc1(i,2)=tmp{1,ind};Acc2(i,2)=tmp{1,ind2};Acc3(i,2)=tmp{1,ind3};Time1(i,2)=tmp{4,ind};Time2(i,2)=tmp{4,ind2};
%        tmp=GraphEncoderEvaluate({Edge,D},Label,opts); Acc1(i,3)=tmp{1,ind};Acc2(i,3)=tmp{1,ind2};Acc3(i,3)=tmp{1,ind3};Time1(i,3)=tmp{4,ind};Time2(i,3)=tmp{4,ind2};
