function [error]=GraphNNEvaluate(X,Y,M,option,num)
...
for i = 1:num
    ...
    if size(M,1)==size(X,1)
        [mdl,filter]=GraphNN(X(train,:),Y(train),M(train,:),option); % build model
        labelV = GraphNNPredict(X(test,:),mdl,filter,M(test,:),option);
    else
        [mdl,filter]=GraphNN(X(train,:),Y(train),0,option); % build model
        labelV = GraphNNPredict(X(test,:),mdl,filter,0,option);
    end
