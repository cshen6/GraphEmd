function [loss_hist, grad_hist, true_loss_hist, true_grad_hist, test_accuracy] = model_2layer_train_and_test(A, X, Y, idx_train, idx_valid, idx_test, szW0, szW1, l2_reg, num_epoch, batch_size, sample_size, learning_algo, optimizer_param)
% All-in-one training and testing for model_2layer.
%
...
W0 = initW(szW0);
W1 = initW(szW1);
w = model_2layer_wrap_param(W0, W1);
...
for epoch = 1:num_epoch
  ...
  [loss_hist(epoch), grad_hist(epoch), true_loss_hist(epoch), ...
   true_grad_hist(epoch), ~, w, optimizer_param] = ...
      model_2layer_train_and_test_one_epoch(...
...
function [loss, grad, true_loss, true_grad, accuracy, w, optimizer_param] = model_2layer_train_and_test_one_epoch(A, X, AX, Y, p, w, szW0, szW1, l2_reg, batch_size, sample_size, learning_algo, optimizer_param, idx, Y_label, update_w, comp_true_loss)
% Subroutine
%
...
if comp_true_loss
  if update_w
    [true_loss, ~, g] = model_2layer_wrapper(A, X, AX, Y, [], [], idx, ...
                                             [], [], w, szW0, szW1, l2_reg);
    true_grad = norm(g);
  else
    [true_loss] = model_2layer_wrapper(A, X, AX, Y, [], [], idx, ...
                                       [], [], w, szW0, szW1, l2_reg);
    true_grad = -1;
  end
else
...
for i = 1:num_batch
  ...
  if update_w
    [f, PP, g] = model_2layer_wrapper(A, X, AX, Y, idx0, idx1, idx2, ...
                                      p0, p1, w, szW0, szW1, l2_reg);
  else
    [f, PP] = model_2layer_wrapper(A, X, AX, Y, idx0, idx1, idx2, ...
                                   p0, p1, w, szW0, szW1, l2_reg);
  end
