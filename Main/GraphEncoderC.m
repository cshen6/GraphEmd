...
function [Z,Y,W,indT,B]=GraphEncoderC(X,k,YT,opts)

if nargin<4
    opts = struct('Laplacian',false,'maxIter',100,'Learn',false,'deg',true,'distance','correlation');
end
...
if ~isfield(opts,'deg'); opts.deg=true; end
...
for r=1:opts.maxIter
    ...
    try
        ...
        %         gmfit = fitgmdist(Z,k,'RegularizationValue',1); % Fitted GMM
...
if nargin<3
    opts = struct('Laplacian',false,'deg',true);
end
if ~isfield(opts,'Laplacian'); opts.Laplacian=false; end
if ~isfield(opts,'deg'); opts.deg=true; end
% opts.deg=false;
